...
void update(ll id, ll lt, ll rt, ll ind) {
    if(lt == rt) tree[id] = 0;
    else {
        ll mid = (lt + rt) / 2;
        if(ind <= mid) update(2 * id, lt, mid, ind);
        else update(2 * id + 1, mid + 1, rt, ind);
        updation(id);
    }
}
...
ll query(ll id, ll lt, ll rt, ll val) {
    ...
    else return query(2 * id + 1, mid + 1, rt, val - tree[2 * id]);
}
